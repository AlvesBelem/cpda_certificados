...
import { EbdAnualCertificateBuilder } from "../_components/ebd-anual-certificate-builder";

export default function CertificadoEbdAnualPage() {
  ...
  return (
    <main className="mx-auto w-full max-w-6xl space-y-6 px-6 py-12 md:px-10 lg:px-16">
      <header className="space-y-4">
        ...
        <h1 className="text-2xl font-semibold text-foreground">Certificado EBD Anual</h1>
        <p className="text-sm text-muted-foreground">
          Use este modelo anual para registrar a conclusão da classe e a promoção do aluno para a próxima turma do próximo ano.
        </p>
      ...
      <Card className="border-border/60 bg-card/80">
        <CardHeader>
          <CardTitle>Informações anuais da EBD</CardTitle>
          <CardDescription>Preencha os dados e veja a prévia do certificado na hora.</CardDescription>
        </CardHeader>
        <CardContent>
          <EbdAnualCertificateBuilder igrejaNome={igrejaNome || "Igreja"} logoUrl={logoUrl || undefined} logoPath={logoPath} />
        </CardContent>
      </Card>
    </main>
  );
