    ...
    <div className="flex h-full flex-col rounded-[32px] bg-white p-6 text-[#6b4b1f] md:p-5">
      <div className="flex flex-col gap-6 md:flex-row md:items-center md:gap-8">
        <div className="flex items-center gap-4 md:flex-1">
          ...
          <div className="space-y-1 text-center md:text-left">
            ...
            <h2 className="text-3xl font-serif text-primary md:text-4xl">Ordenação Diácono</h2>
      ...
      <div className="mt-6 space-y-3 text-sm leading-relaxed text-muted-foreground">
        <p>
          ...
          <span className="font-semibold text-foreground">{localNascimentoTexto}</span>, foi ordenado(a) a Diácono(a) pela Igreja{" "}
...
export function OrdenacaoDiaconoCertificateBuilder({ igrejaNome, logoPath, logoUrl }: BuilderProps) {
  ...
  const { certificateRef, isGenerating, isShareSupported, handleShare, handleGeneratePDF } = useCertificatePDF({
    fileName: `certificado-ordenacao-diacono-${campos.nomeOrdenando || "ordenacao"}.pdf`,
    title: "Certificado de Ordenação Diácono",
    text: `Certificado de ordenação a diácono para ${campos.nomeOrdenando || "membro"}`,
  });
    ...
    <section className="certificate-print-root flex flex-col gap-6 print:block">
      <div className="space-y-6 rounded-3xl border border-border bg-background/70 p-6 shadow-sm print:hidden">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground">Preencha as informações de ordenação a diácono.</p>
        </div>
